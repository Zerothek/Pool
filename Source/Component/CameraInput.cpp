...
void CameraInput::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_UP))	camera->MoveForward(deltaTime);
	if (window->KeyHold(GLFW_KEY_DOWN))	camera->MoveBackward(deltaTime);
	if (window->KeyHold(GLFW_KEY_LEFT))	camera->MoveLeft(deltaTime);
	if (window->KeyHold(GLFW_KEY_RIGHT))	camera->MoveRight(deltaTime);
